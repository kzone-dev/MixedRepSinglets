...
def make_models(T,tmin,tmax):
    ...
    return [cf.Corr2(datatag='Gab', tp=None, tmin=tmin, tmax=tmax, a='a', b='a', dE='dE')]
...
def fit_eigenvalues_file(hdf5file,tmin1,tmin2,tmax1,tmax2,Nmax=10,ensemble="M1"):
    f = h5py.File(hdf5file)
    T = get_hdf5_value(f,ensemble+"/lattice")[0]
    L = get_hdf5_value(f,ensemble+"/lattice")[1]

    ev = get_hdf5_value(f,ensemble+"/eigvals_g5_singlet")[()]
    Delta_ev = get_hdf5_value(f,ensemble+"/Delta_eigvals_g5_singlet")[()]

    eig1 = dict(Gab=gv.gvar(ev[:,17],Delta_ev[:,17]))
    eig2 = dict(Gab=gv.gvar(ev[:,16],Delta_ev[:,16]))

    E1, a1, chi2A, dofA = fit_correlator_without_bootstrap(eig1,T,tmin1,tmax1,Nmax,plotting=PLOT,printing=PRINT)
    E2, a2, chi2B, dofB = fit_correlator_without_bootstrap(eig2,T,tmin2,tmax2,Nmax,plotting=PLOT,printing=PRINT)
...
fit_eigenvalues_file(filename,tmin1=3,tmin2=3,tmax1=10,tmax2=7,Nmax=4,ensemble="M1")
fit_eigenvalues_file(filename,tmin1=3,tmin2=3,tmax1=10,tmax2=9,Nmax=4,ensemble="M2")
fit_eigenvalues_file(filename,tmin1=3,tmin2=3,tmax1=10,tmax2=7,Nmax=4,ensemble="M3")
fit_eigenvalues_file(filename,tmin1=3,tmin2=3,tmax1=10,tmax2=8,Nmax=4,ensemble="M4")